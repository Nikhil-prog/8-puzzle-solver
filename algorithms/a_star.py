...
class ASTAR(BaseAlgo):
    ...
    def __astar_cost__(self, board, depth):
        return depth + self.__heuristic__(board)
    ...
    def run(self):
        ...
        astar_cost = self.__astar_cost__(self.puzzle.initial_board, 0)
        
        # Add initial board to the priority queue
        # as each instance in the form of (astar_cost ,cost, depth, board state, steps)
        priority_queue.put((astar_cost, 0, 0, self.puzzle.initial_board, []))
        ...
        while not priority_queue.empty():
            ...
            for child in neighbhours:
                if child not in visited:
                    ...
                    _child_astar_cost = self.__astar_cost__(child, _child_depth)
                    
                    priority_queue.put((_child_astar_cost, _child_cost, _child_depth, child, _child_step))
